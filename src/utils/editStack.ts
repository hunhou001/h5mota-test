...
export const defineCommand = <T extends any[], R>(command: Command<T, R>) =>
  command;
...
export class EditStack {
  ...
  //     const { stack, current, capacity } = editStack;
  //     const record = command.exec(...args);
  //     const newStack = produce(stack, (stack) => {
  //       if (current < stack.length) {
  //         stack = stack.slice(0, current);
  //       }
